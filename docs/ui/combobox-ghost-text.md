...
基于当前稳固的架构，我们可以轻松地进行以下扩展：

1.  **获取光标前后内容 (支持 Autocomplete)**:
    -   **思路**: Slate 的 `editor` 实例中包含了 `selection` 对象，它精确地描述了光标的位置或用户的选择范围。
    -   **实现**: 我们可以通过 `editor.selection` 来获取光标位置，然后使用 Slate 的 `Editor.string()` 方法提取光标前后的文本。
    -   **示例**:
        ```typescript
        const { selection } = editor;
        if (selection) {
          const textBeforeCursor = Editor.string(editor, { anchor: Editor.start(editor, []), focus: selection.anchor });
          const textAfterCursor = Editor.string(editor, { anchor: selection.anchor, focus: Editor.end(editor, []) });
          
          // 将这些信息传递给回调函数或 AI 服务
          onCursorContextChange({ textBeforeCursor, textAfterCursor });
        }
        ```

2.  **自动调整高度 (Auto-resizing Textarea)**:
    -   **思路**: 当前组件使用固定的 `rows` 属性。要实现类似 `react-textarea-autosize` 的效果，我们需要在内容变化时测量其实际高度。
    -   **实现**: 可以在组件内部维护一个隐藏的、与真实编辑器具有相同宽度和样式的 `<div>`。每次内容更新时，将内容同步到这个隐藏 `<div>` 中，然后测量它的 `scrollHeight`，并将其应用到真实编辑器的高度上。

3.  **更丰富的回调函数**:
    -   **思路**: 开放更多内部事件的钩子，让父组件能更灵活地响应内部状态变化。
    -   **实现**: 可以新增如 `onSuggestionFetch(text: string)`、`onReject()` 等回调 props，在组件内部的相应逻辑点进行调用。

通过这些扩展，`InclusiveGhostTextbox` 可以演变为一个更加通用和强大的 AI 输入组件。 