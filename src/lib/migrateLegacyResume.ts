...
import type { DynamicResumeSection, DynamicSectionItem, ExtendedResumeData } from '@/types/schema';
...
export function needsMigration(data: any): boolean {
  ...
  return 'personalDetails' in data && 
         'sections' in data && 
...
export function migrateLegacyResume(legacyData: any): ExtendedResumeData {
  ...
  const migratedSections: DynamicResumeSection[] = legacyData.sections.map((section: any) => {
