...
export default function ResumeStudioPage() {
  ...
  const isLeftPanelOpen = useHydratedStore(
    useResumeStore,
    (state) => state.isLeftPanelOpen
  );
  const editingTarget = useHydratedStore(
    useResumeStore,
    (state) => state.editingTarget
  );
  const resumeData = useHydratedStore(
    useResumeStore,
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: resumeData
      ? `${resumeData.personalDetails.fullName.replace(/\s+/g, '_')}_Resume`
      : 'Resume',
    ...
    onAfterPrint: () => {
      toast({
        ...
        description:
          'Your resume has been prepared for printing or saving as PDF.',
      });
    },
  ...
  useEffect(() => {
    ...
    if (
      selectedTemplateId &&
      currentResumeData &&
      currentResumeData.templateId !== selectedTemplateId
    ) {
      useResumeStore.getState().updateResumeData((prev) => ({
        ...prev,
          ...
          {isLeftPanelOpen ? (
            ...
            <div className="hidden md:flex flex-col items-center p-2">
              <Button
                ...
                size="icon"
                onClick={() =>
                  useResumeStore.getState().setIsLeftPanelOpen(true)
                }
                className="mt-2"
              >
                <PanelRightOpen />
        ...
        <main className="a4-canvas-container relative flex-grow bg-muted/20">
          {!isLeftPanelOpen && (
            <Button
              variant="ghost"
              size="icon"
              onClick={() => useResumeStore.getState().setIsLeftPanelOpen(true)}
              className="absolute top-4 left-4 z-10 md:hidden no-print"
            >
              <PanelRightOpen />
            </Button>
          )}
