...
> = (set, get) => ({
  ...
  updateResumeData: (updater) =>
    set((state) => ({ resumeData: updater(state.resumeData) })),
  ...
  setIsAutocompleteEnabled: (isEnabled) =>
    set({ isAutocompleteEnabled: isEnabled }),
  toggleAutocomplete: () =>
    set((state) => ({ isAutocompleteEnabled: !state.isAutocompleteEnabled })),
  setIsReviewDialogOpen: (isOpen) => set({ isReviewDialogOpen: isOpen }),
  toggleReviewDialog: (isOpen) =>
    set((state) => ({
      isReviewDialogOpen:
        isOpen !== undefined ? isOpen : !state.isReviewDialogOpen,
    })),
  ...
  exportCurrentSchema: () => {
    ...
    const exportData = {
      ...
      schemaVersion:
        'schemaVersion' in resumeData ? resumeData.schemaVersion : '1.0.0',
