...
export const generateResumeFromContext = ai.defineFlow(
  ...
  async (input: GenerateResumeContextInput): Promise<AIBridgedResume> => {
    ...
    const schemaInstructions =
      AIDataBridge.buildSchemaInstructions(schemaRegistry);
    ...
    if (!output || !output.resumeJson) {
      throw new Error(
        'AI failed to generate a resume. The output was empty or malformed.'
      );
    }
    ...
    if (!validationResult.success) {
      ...
      throw new Error(
        'AI returned JSON that did not match the expected structure.'
      );
    }
