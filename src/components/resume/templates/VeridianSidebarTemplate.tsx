...
export const VeridianSidebarTemplate = ({ resume }: VeridianSidebarTemplateProps) => {
                        ...
                        <ul className="list-none pl-4">
                          {descriptionField.value.map((item, idx) => (
                            <li key={idx} className="relative mb-1.5 pl-4">
                              <span className="absolute left-0 top-0 text-[#1ab0b3] text-[14px] leading-[1.2]">•</span>
                              {item}
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="whitespace-pre-line">{String(descriptionField.value)}</p>
                      )}
                    </div>
                  )}
                </div>
  ...
  const renderSectionForSidebar = (section: RenderableSection) => {
    ...
    switch(section.schemaId) {
      case 'skills':
        return <VeridianBadgeListComponent section={section} roleMap={roleMap} />;
      
      case 'advanced-skills':
        return <VeridianCategorizedSkillsComponent items={section.items} roleMap={roleMap} />;
      ...
      case 'projects':
        return section.items.map(item => (
          <VeridianTitledBlockComponent key={item.id} item={item} roleMap={roleMap} />
        ));
      
      case 'customText':
        return <VeridianSingleTextComponent section={section} roleMap={roleMap} />;
  ...
  const sidebarSchemaIds = ['skills', 'advanced-skills', 'certifications', 'customText'];
