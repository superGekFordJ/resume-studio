...
import { ProClassicMarkdownRenderer } from '../rendering/pro-classic/ProClassicMarkdownRenderer';
  ...
  const renderSection = (section: RenderableSection) => {
    ...
    switch (section.schemaId) {
      ...
      case 'customText':
        ...
        const summaryField = pickFieldByRole(
          summaryItem,
          'description',
          roleMap
        );
        const summaryContent = summaryField?.value
          ? Array.isArray(summaryField.value)
            ? summaryField.value.join('\n')
            : summaryField.value
          : '';
        return (
          <ProClassicMarkdownRenderer className="py-1.5 px-3">
            {summaryContent}
          </ProClassicMarkdownRenderer>
        );
      case 'skills':
        return (
          <SimpleSkillsComponent items={section.items} roleMap={roleMap} />
        );
      case 'advanced-skills':
        return (
          <CategorizedSkillsComponent items={section.items} roleMap={roleMap} />
        );
      case 'experience':
      ...
      case 'education':
        return section.items.map((item) => (
          <EducationItemComponent key={item.id} item={item} roleMap={roleMap} />
        ));
      case 'projects':
        return section.items.map((item) => (
