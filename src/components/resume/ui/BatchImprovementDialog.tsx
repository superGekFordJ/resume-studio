...
function ItemDiff({
  ...
  const getItemTitle = (): string => {
    ...
    const subtitleField = sectionSchema.fields.find(
      (f) =>
        f.id === 'company' ||
        f.id === 'organization' ||
        f.id === 'institution' ||
    ...
    const titleValue = original.data?.[titleField?.id || ''] as
      | string
      | undefined;
    const subtitleValue = original.data?.[subtitleField?.id || ''] as
      | string
      | undefined;
  ...
  return (
    <AccordionPrimitive.Item
      value={`item-${index}`}
      className="border rounded-lg mb-2 bg-white data-[state=open]:shadow-md"
    >
...
export default function BatchImprovementDialog() {
  ...
  const rejectBatchImprovement = useResumeStore(
    (state) => state.rejectBatchImprovement
  );
                ...
                <div className="space-y-2">
                  ...
                  >
                    {originalItems.map((originalItem, index) => {
                      ...
                      if (
                        !improvedItemData ||
                        JSON.stringify(originalItem.data) ===
                          JSON.stringify(improvedItemData)
                      )
                        return null;
