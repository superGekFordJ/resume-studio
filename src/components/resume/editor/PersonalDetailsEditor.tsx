...
export default function PersonalDetailsEditor({ personalDetails }: PersonalDetailsEditorProps) {
  const updateField = useResumeStore(state => state.updateField);
  const resumeData = useResumeStore(state => state.resumeData);
  const isAutocompleteEnabled = useResumeStore(state => state.isAutocompleteEnabled);
  
  const handleFieldChange = (fieldName: keyof PersonalDetails, value: string) => {
    updateField({
      sectionId: '',
      fieldId: fieldName,
      value,
      ...
      <div className="mb-6">
        ...
        <div className="mt-2">
          <AvatarUploader
            ...
            onChange={(value: string | undefined) => handleFieldChange('avatar', value || '')}
          />
        </div>
      </div>
