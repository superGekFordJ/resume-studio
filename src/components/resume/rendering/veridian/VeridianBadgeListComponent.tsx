...
export const VeridianBadgeListComponent = ({ section, roleMap }: VeridianBadgeListComponentProps) => {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {section.items.map(item => {
        ...
        const primaryField = pickFieldByRole(item, 'skills', roleMap) ||
                           pickFieldByRole(item, 'title', roleMap) || 
                           item.fields[0];
        if (!primaryField?.value) return null;

        const values = Array.isArray(primaryField.value) ? primaryField.value : [primaryField.value];
